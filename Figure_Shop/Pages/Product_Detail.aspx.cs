        ...
        protected void DataList1_ItemCommand(object source, DataListCommandEventArgs e)
        {
            if (e.CommandName.ToString() == "Add_To_Cart")
            {
                if (Session["user_email"] != null)
                {
                    ...
                    string userId = connect.getField("select id from users where email = '" + Session["user_email"] + "'");
                    string magh = connect.getField("select ma_gio_hang from gio_hang where user_id = " + userId);
                    DataTable sp = connect.getData("select * from chi_tiet_gh where ma_gio_hang = " + magh + " and ma_sp = " + ma_sp);
